...
class ETLPipeline:
    ...
    def run(self):
        data, schema = Extractor.extract(
            api=self.extractor.api,
            **self.extractor.arguments
        )
        transformed_data = Transformer.transform(
            type=self.transformer.type,
            data_raw=data,
            data_schema=schema
            )
        return Loader.load(
    ...
    return jsonify([
        {
            ...
            "description": "" # TODO: Add a description from the own class
        },
    ])
...
def yahoofinance_get_data():
    args = request.args.to_dict()  # TODO: Find a better way to handle this only sending the needed arguments
